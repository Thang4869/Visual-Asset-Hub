...
- **Dịch vụ đã đăng ký:** CORS (AllowAll), Controllers, Swagger, EF Core với SQLite
- **Logic khởi động:**
  - `EnsureCreated()` — tự động tạo DB schema
  - **Seed 3 collection mặc định** nếu chưa có:
    1. `Images` (type=`image`, color=`#007bff`, order=1)
    2. `Links` (type=`link`, color=`#28a745`, order=2)
    3. `Colors` (type=`color`, color=`#ffc107`, order=3)
- **Middleware pipeline:** CORS → Swagger (dev only) → StaticFiles (`wwwroot/`) → Authorization → MapControllers
...
| `GET` | `/api/Collections/{id}/items?folderId=` | Lấy collection cùng items (lọc theo folderId) + subcollections. Trả về `CollectionWithItems`. Items sắp xếp: thư mục trước → SortOrder → FileName |
...
- **Quản lý schema:** `EnsureCreated()` — tự tạo khi chạy lần đầu, không dùng migrations
- **Seeding:** 3 collections mặc định (Images, Links, Colors) nếu bảng trống
...
|---------------------------------------|--------------|---------------|----------------|--------|
| **GĐ 1 — Production cơ bản** | 7 | 4 | 3 | 57% |
...
#### Giai đoạn 1 — Còn thiếu (3/7 hạng mục)

| # | Hạng mục | Mức ưu tiên | Lý do chưa làm | Hậu quả |
|---|----------|-------------|-----------------|---------|
| 1 | **Authentication (JWT + Identity)** | 🔴 CRITICAL | Phức tạp, cần thêm nhiều entity + middleware + frontend login flow | Không thể deploy public. Bất kỳ ai biết URL đều CRUD được mọi data |
| 2 | **EF Core Migrations** | 🔴 HIGH | Đang dùng `EnsureCreated()` — cần chuyển sang `dotnet ef migrations` | Thay đổi model = xóa DB = mất toàn bộ data. Không quản lý schema version |
| 3 | **User Entity + Data Ownership** | 🔴 HIGH | Phụ thuộc Authentication | Không phân biệt được ai sở hữu tài nguyên nào, không multi-user |
...
| Security | 4/10 | Rate limiting + CORS config tốt, nhưng ZERO authentication = hard blocker |
...
|------------|-------|---------------------|
| Backend Controllers | 4 | ~280 |
| Backend Services | 3 interfaces + 3 implementations | ~520 |
| Backend Models/DTOs | 3 | ~170 |
| Backend Middleware | 1 | ~83 |
| Backend Data | 1 | ~100 |
| Backend Config | Program.cs | ~102 |
| **Tổng Backend** | **~15 files** | **~1,255** |
...
|---------|--------------|--------|-----------|
| 🔴 1 | **Chuyển sang EF Core Migrations** — Xóa `EnsureCreated()`, dùng `dotnet ef migrations add/update` | Low | Không còn mất data khi đổi schema |
| 🔴 2 | **Thêm Authentication** — JWT + ASP.NET Identity, login/register endpoints, `[Authorize]` | High | Mở cánh cửa deploy public, multi-user |
| 🔴 3 | **User Entity + UserId trên Asset/Collection** | Medium | Data ownership, multi-user isolation |
| 🟡 4 | **React Router** — `/collections/:id`, `/search?q=`, deep linking | Medium | URL shareable, browser back/forward |
| 🟡 5 | **Loading Skeleton / Empty States** | Low | UX mượt hơn, professional hơn |
| 🟡 6 | **Responsive breakpoints** | Medium | Mobile/tablet support |
| 🟡 7 | **Unit Tests** — xUnit + Moq cho services, Jest cho hooks | Medium | Refactor an toàn |
| 🟢 8 | **Docker + docker-compose** | Low | Deploy nhất quán |
| 🟢 9 | **Serilog structured logging** | Low | Log searchable, aggregatable |
| 🟢 10 | **Thumbnail generation** | Medium | Giảm bandwidth, tải nhanh hơn |

---

### 9.8 KẾT LUẬN

Dự án Visual Asset Hub đã tiến xa từ trạng thái MVP ban đầu. So với đánh giá kiến trúc (ARCHITECTURE_REVIEW.md), **~65% các khuyến nghị đã được hiện thực hóa** — đặc biệt mạnh ở phần kiến trúc backend (Service Layer, Storage Abstraction, Exception Handling, Rate Limiting, Indexing) và frontend (Hooks, API layer, ErrorBoundary).

**Rào cản lớn nhất còn lại** là Authentication — đây là điều kiện tiên quyết để deploy ra bất kỳ môi trường nào ngoài localhost. EF Core Migrations là việc tiếp theo cần làm ngay để bảo vệ dữ liệu khỏi bị mất khi thay đổi schema.

Hệ thống hiện tại phù hợp cho:
- ✅ Demo / Prototype
- ✅ Phát triển local cho 1 developer
- ⚠️ Internal tool (nếu nằm sau VPN/firewall)
- ❌ Public deployment (thiếu auth)
- ❌ Multi-user SaaS (thiếu auth + user isolation)
